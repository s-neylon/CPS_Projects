...
New version of file created, to have all colors be the same.
...
occ_plot <- occ_groups %>% 
  ...
  mutate(Occupation = fct_reorder(Occupation, NYC_pctUNION)) %>% 
  select(USApctUNION, NYC_pctUNION, Occupation) %>% 
  rename(USA = USApctUNION, "NYC MSA" = NYC_pctUNION) %>% 
  pivot_longer(c(USA, "NYC MSA"), names_to = "geo", values_to = "union_pct") %>% 
  mutate(union_pct = round(union_pct, 1),
         geo = factor(geo, levels = c("USA", "NYC MSA"))) %>% 
  arrange(union_pct) %>% 
  ggplot(mapping = aes(x = Occupation, y = union_pct, fill = geo)) +
  geom_col(position = "dodge") +
...
#  geom_text(aes(label = union_pct), position = position_dodge(width = 0.9), hjust=-0.5) +
  ...
  labs(x= element_blank(), y=element_blank(),
       title="Industries in the New York City Metro Area",
       subtitle="Comparing Total with Unionized Employment in NYC MSA, 2019-2022",
       fill = "Employment") + 
  scale_x_discrete(labels = function(x) str_wrap(x, width = 40)) +
  scale_fill_manual(values = c("#CC6677", "#4477AA"), labels = c("Unionized", "Total")) +
  #theme_minimal() +
  theme_ipsum(grid = FALSE,
              plot_margin = margin(10, 10, 10, 10),
              plot_title_margin = 5,
